...
impl < ColIndex, RowIndex, Coefficient >
    ...
    where   ColIndex:       Clone + Hash + std::cmp::Eq,
            RowIndex:       Clone + Hash + std::cmp::Eq, 
            Coefficient:    std::cmp::PartialEq,

{
    fn eq(&self, other: &Self) -> bool {
        self.bimap_min == other.bimap_min && self.bimap_maj == other.bimap_maj && self.vec_snzval == other.vec_snzval
    }
}    
    ...
    
{
    ...
    /// Returns a generalized inverse of `self`
    /// 
    /// The generalized inverse is computed by transposing `self` and inverting every nonzero entry.
    /// 
    /// This function should only be used when the structural nonzero entries of the matrix are in fact nonzero.
    /// 
    /// # Examples
    /// 
    /// ```
    ...
    /// 
    /// // define the generalized matching matrix
    /// let mut matching = GeneralizedMatchingArrayWithMajorOrdinals::new();
    /// matching.push(0, 1, Ratio::new(1,33));
    /// matching.push(2, 3, Ratio::new(1,99));
    /// 
    /// // this is what the method *should* return as the generalized inverse
    /// let mut generalized_inverse = GeneralizedMatchingArrayWithMajorOrdinals::new();
    /// generalized_inverse.push(1, 0, Ratio::new(33,1));
    /// generalized_inverse.push(3, 2, Ratio::new(99,1));
    /// 
    /// 
    /// // check that the matrix returned by .generalized_inverse(ring_operator) is correct
    /// let ring_operator = DivisionRingNative::< Ratio<i64> >::new();        
    /// assert_eq!( generalized_inverse, matching.generalized_inverse(ring_operator)  );
    /// ```
    pub fn generalized_inverse< RingOperator >( & self , ring_operator: RingOperator) 
        -> GeneralizedMatchingArrayWithMajorOrdinals< RowIndex, ColIndex, Coefficient > 
        where
            Coefficient:        Clone,
            RingOperator:       DivisionRing< Coefficient >
        {
            let vec_snzval    =   self.vec_snzval
                                            .iter()
                                            .cloned()
        ...
        GeneralizedMatchingArrayWithMajorOrdinals{
            bimap_maj:          self.bimap_min.clone(),
            bimap_min:          self.bimap_maj.clone(),
            vec_snzval
        }
    }    
    ...
    /// The nonzero snzval associated with this `keymaj` index, returned as an Option< Coefficient >
    pub fn keymaj_to_snzval_opt( &self, keymaj: & RowIndex ) -> Option< Coefficient >
        where Coefficient: Clone    
    {
        self.keymaj_to_ord( keymaj )
            .map( 
                | ord | 
                self.vec_snzval[ ord ].clone()
            )
    }    
