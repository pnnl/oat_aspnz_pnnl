...
/// ```
pub fn interval_decomposition_for_zigzag_of_cospans<RingOperator, RingElement>(
    cospans: Vec<Cospan<RingElement>>,
    ...
    RingElement: Clone + std::fmt::Debug + Eq + Ord,
{
    ...
    let vector_space_dimensions = homology_cycle_bases
        .iter()
        .map(|basis| basis.number_of_rows())
        .collect::<Vec<_>>();
    ...
    if print_profiling_statistics{ println!("Time elapsed to build quiver: {:?}", duration); }
