...
use crate::utilities::order::{ JudgePartialOrder, OrderOperatorByKey, OrderOperatorByKeyCustom, JudgeOrder };
...
//                                                 )
//                                             >;
type OrderOperatorSimplexFiltered< FilVal, Coefficient > =   OrderOperatorByKeyCustom< 
    ...
    fn check_that_some_basic_functions_run_without_error() {
        ...
        for keymaj in iter_keymaj.clone() {
            assert!( is_sorted_strictly(    &chain_complex_ref.view_minor_descend(keymaj).iter().cloned().collect_vec() , 
                                            &OrderOperatorByKeyCustom::new( OrderOperatorAutoReverse::new() )  // NOTE THAT HERE WE USE GT 
                                        ) );
        }    
