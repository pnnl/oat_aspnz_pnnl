...
{ unreversed: UnreversedOrderOperator }
...
impl < UnreversedOrderOperator > 
    ...
    ReverseOrder
        < UnreversedOrderOperator > 
{ 
    pub fn new( unreversed: UnreversedOrderOperator ) ->  ReverseOrder< UnreversedOrderOperator > { ReverseOrder{ unreversed } }

    /// Returns the inner order operator (unreversed)
    pub fn unreversed( self ) -> UnreversedOrderOperator { self.unreversed }
}
...
{
    fn judge_partial_cmp( & self, lhs: & T, rhs: & T ) -> Option<Ordering> { self.unreversed.judge_partial_cmp( rhs, lhs ) }
}
...
{
    fn judge_cmp( & self, lhs: & T, rhs: & T ) -> Ordering { self.unreversed.judge_cmp( rhs, lhs ) }
}
...
/// See See the documentation for the [order](crate::utilities::order) module for an overview on order operators.
#[derive(Debug, Eq, PartialEq)]
pub struct OrderOperatorByKeyCustom < Key, Val, KeyValPair, KeyComparator > 
{
...
// Constructor
impl< Key, Val, KeyValPair, KeyComparator > OrderOperatorByKeyCustom < Key, Val, KeyValPair, KeyComparator >  {
    pub fn new( key_comparator: KeyComparator ) -> OrderOperatorByKeyCustom < Key, Val, KeyValPair, KeyComparator >  {
        OrderOperatorByKeyCustom{ key_comparator, phantom_key: PhantomData, phantom_pair: PhantomData, phantom_val: PhantomData }
    }
}
...
impl< Key, Val, KeyValPair, KeyComparator >
    ...
    where 
        KeyComparator:  Clone,
{
    fn clone(&self) -> Self { OrderOperatorByKeyCustom::new( self.key_comparator.clone() ) }
} 
