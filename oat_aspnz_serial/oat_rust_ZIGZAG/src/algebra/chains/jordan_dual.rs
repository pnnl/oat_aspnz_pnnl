...
use crate::algebra::matrices::operations::umatch::row_major::comb::{CombCodomainInvViewMajorAscend, CombCodomainViewMajorAscend, CombCodomainViewMinorDescend, CombDomainInvViewMajorAscend, CombDomainViewMinorDescend};
...
pub struct DualJordanBasisMatrixVector< 
                    'a, 
                    Mapping, 
                    RingOperator, 
...
{
    row:     IterTwoType<     
                    CombCodomainInvViewMajorAscend<'a, Mapping, RingOperator, >,
                    CombDomainInvViewMajorAscend< Mapping, RingOperator, OrderOperatorRowEntries >,
                >,
}
        ...
        EntryBoth:                                  std::cmp::PartialEq                  
{
    ...
    fn next(&mut self) -> Option<Self::Item> { self.row.next() }
}
...
pub struct DualJordanBasisMatrix< 
                    'a, 
                    Mapping, 
                    RingOperator, 
                    OrderOperatorRowEntries, 
...
impl <  'a,
    ...
    where
        Mapping:                               ViewRowAscend<EntryMajor = EntryBoth> + 
                                                    ViewColDescend<EntryMinor = EntryBoth> + 
                                                    IndicesAndCoefficients< ColIndex=KeyBoth, RowIndex=KeyBoth >,     
        KeyBoth:                                    'a + Clone + Debug + Hash + std::cmp::Eq, // required for the hashing performed by the generalized matching array   // !!!! try deleting debug
        Mapping::Coefficient:                       Clone + Debug,
        Mapping::ViewMajorAscend:              IntoIterator,        
        Mapping::EntryMajor:         Clone + KeyValSet< Mapping::RowIndex, Mapping::Coefficient > + KeyValNew< Mapping::RowIndex, Mapping::Coefficient >, 
        Mapping::ViewMinorDescend:             IntoIterator,        
        ...
        EntryBoth:                                  std::cmp::PartialEq  
{
    type ViewMajorAscend = DualJordanBasisMatrixVector< 'a, Mapping, RingOperator, OrderOperatorRowEntries, KeyBoth, EntryBoth, >;
    ...
    fn   view_major_ascend( &self, index: Self::ColIndex ) -> Self::ViewMajorAscend {
        ...
        let keymin = index;
        let chain = 
            match self.umatch.matching_ref().keymin_to_keymaj( &keymin ) {
                None => {
                    // the cochain never becomes a coboundary, so look it up as a row of the inverse codomain comb
                    IterTwoType::Iter1(
                        self.umatch.comb_codomain_inv()
                        .view_major_ascend( keymin.clone() )
                    )

                }
                _ => {             
                    // the chain eventually becomes a coboundary, so look it up as a row of the inverse domain comb
                    let boundary_vec        =   self.umatch.comb_domain_inv()
                                                        .view_major_ascend( keymin );
                    IterTwoType::Iter2( boundary_vec )
                }
            };
        DualJordanBasisMatrixVector{ row: chain }
    }
}
