...
use crate::utilities::order::{JudgePartialOrder, OrderOperatorAuto, OrderOperatorByKey, OrderOperatorByKeyCustom};
...
use super::jordan_dual::{DualJordanBasisMatrix, DualJordanBasisMatrixVector};
...
pub fn factor_boundary_matrix< 
        ...
        ->  FactoredBoundaryMatrix< 
                    Mapping, 
                    RingOperator, 
                    OrderOperatorByKeyCustom< KeyBoth, Mapping::Coefficient, EntryBoth, OrderOperatorKeyBoth >, 
                    KeyBoth,
                    EntryBoth,                    
                    I,                     
                > 
...
impl < 
    ...
    FactoredBoundaryMatrix< 
                    Mapping, 
                    RingOperator, 
                    OrderOperatorByKeyCustom< KeyBoth, Mapping::Coefficient, EntryBoth, OrderOperatorAuto >, 
                    KeyBoth,
                    EntryBoth,                    
                    I, 
                > 
...
{    
    /// Harmonic elements in the Jordan basis, formatted as a [VecOfVec](crate::algebra::matrices::types::vec_of_vec::sorted::VecOfVec)
    /// 
    /// Each row of the [VecOfVec] represents a basis vector. Basis vectors appear in the same order they are returned by [basis_harmonic].
    /// 
    ...
    pub fn basis_homology_vec_of_vec( &self ) -> 
        VecOfVec< KeyBoth, Mapping::Coefficient >
    { 
        let basis_vector_iter       =   self.basis_harmonic()
                                            .map(
                                                |basis_vector|
                                                {
                                                    let mut basis_vector    =   basis_vector.map(|entry| (entry.key(), entry.val()) ).collect::<Vec<_>>();
                                                    basis_vector.reverse(); // we have to reverse the order of entries because the jordan basis vectors return entries in reverse order
                                                    basis_vector
                                                }
                                            )
                                            .collect();
        VecOfVec::new( basis_vector_iter )
    }   


    /// Harmonic elements in the dual Jordan basis, formatted as a [VecOfVec](crate::algebra::matrices::types::vec_of_vec::sorted::VecOfVec)
    /// 
    /// Each row of the [VecOfVec] represents a basis vector. Basis vectors appear in the same order they are returned by [basis_harmonic].
    /// 
    /// The set of dimension-`d` chains returned by this iterator is a basis of cycle representatives for homology in dimension `d`,
    /// provided that `self.row_indices()` runs over all indices of the boundary matrix of dimension-`d` and dimension-`d-1`.
    pub fn basis_cohomology_vec_of_vec( &self ) -> 
        VecOfVec< KeyBoth, Mapping::Coefficient >
    { 
        let jordan_basis_matrix_dual       =   self.jordan_basis_matrix_dual();
        let mut basis_vectors           =   Vec::new();
        for index in self.indices_harmonic() {
            let basis_vector        =   jordan_basis_matrix_dual
...
{    
    ...
    pub fn jordan_basis_matrix_dual( &self ) -> DualJordanBasisMatrix< '_, Mapping, RingOperator, OrderOperatorBoth, KeyBoth, EntryBoth, >  
    { DualJordanBasisMatrix::new( & self.umatch ) }
