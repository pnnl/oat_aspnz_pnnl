        ...
        RingElement:        Clone + std::fmt::Debug + Eq + Ord,
{
    ...
    if n_hypergraphs == 0 { // if there are no hypergraphs then short circuit
        ...
        for basis_vector in factored.basis_homology_vec_of_vec().vec_of_vec() {
            let dimension = chain_dimension(basis_vector);
            let leftmost_vertex = 0;
            let submodule   =   IntervalSubmoduleBasis::new( leftmost_vertex , vec![basis_vector.clone()] );
...
pub fn interval_decomposition_for_zigzag_of_hypgeraph_unions_WITH_SPANS< RingOperator, RingElement >( 
        hypergraphs:                Vec< Vec< Vec< usize > > >, 
        ring_operator:              RingOperator,
        max_homology_dimension:     usize,
    )
