...
use crate::{algebra::{matrices::{operations::{invert::InverseOfTriangularArrayLazyAscending, umatch::row_major::Umatch}, query::{MatrixOracle, ViewRowAscend}, types::vec_of_vec::sorted::VecOfVec}, rings::operator_traits::DivisionRing, vectors::operations::VectorOperations}, utilities::order::{OrderOperatorAuto, OrderOperatorByKey}};
                ...
                < RingOperator, RingElement > 
{
    ...
    matrices:                       Vec< VecOfVec<usize, RingElement> >,
...
impl < RingOperator, RingElement > 
    ...
    where 
        RingOperator:               Clone + DivisionRing< RingElement >,
        RingElement:                Clone + Debug,        
{
    ...
    pub fn arrow_matrices( &self ) -> & Vec< VecOfVec<usize, RingElement> > {
        & self.matrices
    }
    ...
    pub fn diagonalize( &self ) -> 
        ...
        for p in 0 .. n_arrows {
            ...
            let umatch                      =   Umatch::factor(
                                                            & X, // mapping: Mapping,
                                                            ( 0 .. X.number_of_rows() ).rev(), // iter_keymaj: IterRowIndex,
                                                            self.ring_operator(),
                                                            OrderOperatorAuto, //order_operator_keymaj: OrderOperatorRowIndices,
                                                            OrderOperatorAuto, // order_operator_keymin: OrderOperatorColIndex
                                                        );
            ...
            let get_row                                 =   | row_index | -> Vec<(usize,RingElement)> {
                                                                let mut row    =   umatch
                                                                    .comb_domain_inv()
                                                                    .view_major_ascend( row_index )
                                                                    .collect::<Vec<_>>();
                                                                if let Some( scalar )   =   umatch.matching_ref().keymin_to_snzval_opt( & row_index ) {
            ...
            let Sinv_scaled                             =   row_index_iterator  
                                                                .map(|i| get_row(i) )
                                                                .collect::<Vec<_>>();
            let Sinv_scaled                             =   VecOfVec::new ( Sinv_scaled );
            ...
            for ( right_basis_index, bar_ledger ) in bars_anchored_at_p.iter().enumerate() {
                ...
                let linear_combination_right            =   ( & basis_update_for_space_p )
                                                            .view_major_ascend( right_basis_index )
                                                            .collect::<Vec<_>>();
            ...
            if  we_will_need_a_copy_of_Bp1_inverse {
                ...
                let Bp1_inv_lazy                        =   InverseOfTriangularArrayLazyAscending::new( 
                                                                & B[ p + 1 ], 
                                                                self.ring_operator(),
                                                                OrderOperatorByKey::new(),
                                                            ); 
                ...
                let Bp1_inv_rows                        =   ( 0 .. dimensions[ p + 1] ).map( 
                                                                | i |
                                                                Bp1_inv_lazy.view_major_ascend( i )
                                                            );
            ...
            let matching_matrix                         =   umatch.matching_ref();  
            ...
            let basis_vector_in_p_is_matched_in_p1      =   | i: usize | ->  bool  {
                                                                if directions[ p ] {
                                                                    matching_matrix.contains_keymaj( & i )
                                                                } else {
                                                                    matching_matrix.contains_keymin( & i )                                                                
                                                                }
                                                            };                                                                           
            ...
            if we_will_need_a_copy_of_Bp1_inverse {
                ...
                let column_reindexing_function          =   | i: &usize | -> usize { inverse_permutation[*i].clone() };

                Binv.entry(p+1).and_modify(| m | {
                    *m                                  =   (*m).reassign_column_indices_out_of_place( & column_reindexing_function ).unwrap();
                });
            }
    ...
    pub fn validate_diagonalization(
            & self,
            diagonalization:    & Diagonalization< RingElement >,
        )
        ->
        Result<
            (),
