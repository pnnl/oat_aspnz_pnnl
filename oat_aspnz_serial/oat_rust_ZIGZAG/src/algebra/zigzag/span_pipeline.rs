...
use crate::algebra::{chains::factored::factor_boundary_matrix, rings::operator_traits::DivisionRing}; 
...
use super::{spans::{factor_dowker_complex, induced_span}, decompose::{QuiverReprsentation, SingleBarBasisVectorIndexLedger}};
...
/// Returns an interval decomposition of the zigzag module of a sequence of spans, where each span has an associated basis of simplicial chains
/// 
/// **This method consumes the spans**, because we place them in a quiver representation.
/// 
...
/// ```
pub fn interval_decomposition_for_zigzag_of_spans< RingOperator, RingElement >( 
        spans:                     Vec< Span< RingElement > >, 
    ...
    where
        RingOperator:       Clone +  DivisionRing< RingElement > + std::fmt::Debug,
        RingElement:        Clone + std::fmt::Debug + Eq + Ord,
{
    // compute number of vertices and arrows    
    if spans.len() == 0 { // if there are no hypergraphs then short circuit
        return Vec::with_capacity(0);
    }    
    let n_spans                                         =   spans.len();
    let n_arrows                                        =   2 * n_spans;
    let n_vertices                                      =   2 * n_spans + 1.min( spans.len() ); // the term 1.min( spans.len() adds 1 if there is at least one span in the list; technically we don't need to worry about this because we can't get to this poitn in the code unless there is at least one span ... but it makes me feel better to have formulae that work everywhere
    ...
    let arrow_directions                                =   ( 0 .. n_arrows ).map(|i| i.is_odd() ).collect::<Vec<_>>();    
    ...
    homology_cycle_bases.push( spans[0].left_basis_opt.clone().unwrap() );
    
    
    // fill in matrices, vector space grades, and gradings
    for span in spans {      
        matrices.push( span.left_morphism );
        matrices.push( span.right_morphism );
        homology_cycle_bases.push( span.center_basis );
        homology_cycle_bases.push( span.right_basis );            
    }
    ...
    for single_bar_ledger in diagonalization.list_of_single_bar_basis_vector_index_ledgers() {
        ...
        for (vertex, basis_vector_index) in single_bar_ledger.iter() {
            let usize_row       =   diagonalization.bases()[ vertex ].row_ref( basis_vector_index );

            let chain   =   vector_matrix_multiply_major_ascend_simplified(
                                                                usize_row, 
